	...
	var smoke = {
	  ...
	  i: 0,
		...
		build: function (e, f) {
			...
			if (f.type === 'prompt') {
				prompt = 
					'<div class="dialog-prompt">'+
						'<input class="cnpj_cpf_dialog" id="dialog-input-'+f.newid+'" maxlength="14" type="text" ' + (f.params.value ? 'value="' + f.params.value + '"' : '') + ' />'+
					'</div>';
			...
			//
			if (posmodel.config.iface_vkeyboard){
				posmodel.pos_widget.onscreen_keyboard.connect($('.cnpj_cpf_dialog'));
				//$('li:nth-child(28)').trigger('click');
				if (posmodel.pos_widget.onscreen_keyboard.state === 'alphanumeric'){
	            	posmodel.pos_widget.onscreen_keyboard.toggleNumLock();
	            	$('li.clear_input').attr('style', 'display: list-item');
	            }
				$('.pos .keyboard .lastitem').attr('style', 'margin-right: 5px !important');
	            $('li:nth-child(n+11)').hide();
	            $('li:nth-child(27)').show();
	            $('li:nth-child(28)').show();
	            $('.pos .simple_keyboard li').css({"width": "122px","height": "69px", "line-height": "69px"});
			}
		...
		finishbuildPrompt: function (e, f, box) {
			...
			smoke.listen(
				...
				function () {
					...
					posmodel.pos_widget.onscreen_keyboard.hide();
					posmodel.pos_widget.onscreen_keyboard.toggleNumLock();
		            $('li:nth-child(n+11)').show();
		            $('.pos .simple_keyboard li').css({"width": "49px","height": "49px", "line-height": "49px"});
		            $('.pos .keyboard .lastitem').attr('style', 'margin-right: 0px !important');
		            $('li.clear_input').attr('style', 'display: none');
		            $('li.space').css({"width": "275px"});
		            $('li.numlock').css({"width": "105px"});
				}
			);
			...
			//yogesh modified for input only integers
