...
class pos_order(models.Model):
    ...
    def _amount_line_tax(self, cr, uid, line, context=None):
        ...
        taxes_ids = [tax for tax in line.line_taxes_ids if
                     tax.company_id.id == line.order_id.company_id.id and not tax.base_code_id.tax_discount]
    ...
    def create_from_ui(self, cr, uid, orders, context=None):
        ...
        company_taxes = [tax.tax_id.id for tax in
                         self.pool.get('res.users').browse(cr, uid, uid).company_id.product_tax_definition_line]
            ...
            if 'data' in tmp_order.keys():
                for line in tmp_order['data']['lines']:
                    ...
                    if 'product_id' in line[2].keys():
                        ...
                        product_taxes = [tax.id for tax in
                                         self.pool.get('product.product').browse(cr, uid, product_id).taxes_id]
...
class pos_order_line(models.Model):
    ...
    def tko_onchange_product_id(self, cr, uid, ids, pricelist, product_id, qty=0, partner_id=False, context=None):
        result = self.onchange_product_id(cr, uid, ids, pricelist, product_id, qty=qty, partner_id=partner_id,
                                          context=context)
