...
class res_partner_email(osv.osv):
    ...
    _sql_constraints = [('unique_email', 'unique(email)', u"e-mail deve ser único!"), ]
    ...
    _constraints = (_ValidateEmail, 'Error!\nPlease enter a valid email address.', ['email']),
    
    def set_partner_email(self, cr, uid, ids, context=None):
        res = {}
        for record in self.browse(cr, uid, ids):
            email_ids = self.search(cr, uid, [('res_partner_id', '=', record.res_partner_id.id)])
