...
class pos_config(models.Model):
    ...
    discount_card_ids = fields.Many2many(
        'pos.discount.cards', string='Discount cards')
...
class pos_session(models.Model):
    ...
    discount_card_ids = fields.Many2many(
        'pos.discount.cards',
        'pos_session_cards_rel',
        'session_id',
        'card_id',
        related='config_id.discount_card_ids',
        readonly=True,
        string='Available Discount Cards')
        ...
        if len(order_ids) == len(orders):
            ...
            for tmp_order in orders:
                if 'data' in tmp_order.keys():
                    discount_card_id = tmp_order['data'].get(
                        'discount_card_id', False)
                    if discount_card_id:
                        card = card_obj.browse(cr, uid, int(discount_card_id))
                        discount_value = card.value
                        if card.type == 'p':
                            # get total from record
                            total = pos_obj.browse(
                                cr, uid, order_ids[i]).amount_total
                    ...
                    if pos_obj.test_paid(cr, uid, [order_ids[i]]):
                        pos_obj.signal_workflow(
                            cr, uid, [order_ids[i]], 'paid')
                    i = i + 1
        return order_ids
...
class pos_order_line(models.Model):
    ...
    line_subototal = fields.Float(
        string='Subtotal',
        compute='_get_line_subtotal',
        store=True)

    @api.one
    @api.depends('price_unit', 'discount', 'qty')
    def _get_line_subtotal(self):
        self.line_subototal = self.price_unit * \
            (1 - (self.discount or 0.0) / 100.0) * self.qty
