...
class res_partner(osv.osv):
    ...
    def create(self, cr, uid, vals, context=False):
        ...
        phone_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', res), ('type_id', '=' , phone_type_id[0])])
        mobile_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', res), ('type_id', '=' , mobile_type_id[0])])
        if phone_ids:
    ...
    def _get_phones(self, cr, uid, ids, name, args, context=False):
        ...
        for record in self.browse(cr, uid, ids):
            ...
            mobile_ids = []
            if phone_type_id:
                phone_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', record.id), ('type_id', '=' , phone_type_id[0]), ('is_active', '=', True)], order='id desc', limit=1,)
            if mobile_type_id:
                mobile_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', record.id), ('type_id', '=' , mobile_type_id[0]), ('is_active', '=', True)], order='id desc', limit=1,)
            if len(phone_ids):
                res[record.id]['phone'] = phone_obj.browse(cr, uid, phone_ids[0]).phone
            if len(mobile_ids):
                res[record.id]['mobile'] = phone_obj.browse(cr, uid, mobile_ids[0]).phone
        return res
    ...
    def _set_phone_id(self, cr, uid, ids, name, value, args, context=None):
        ...
        for record in self.browse(cr, uid, ids):
            if value and ids:   
                for record in self.browse(cr, uid, ids):
                    phone_type_id = self.pool.get('res.partner.phone.type').search(cr, uid, [('code', '=', 'phone')])
                    phone_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', record.id), ('phone', '=', value), ('type_id', '=' , phone_type_id[0])])
                
                if phone_ids:
                    previous_phone_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', record.id), ('type_id', '=' , phone_type_id[0])])
                    phone_obj.write(cr, uid, previous_phone_ids, {'is_active' : False})
                    phone_obj.write(cr, uid, phone_ids, {'is_active' : True})
    ...
    def _set_mobile_id(self, cr, uid, ids, name, value, args, context=None):
        ...
        for record in self.browse(cr, uid, ids):
            if value and ids:   
                for record in self.browse(cr, uid, ids):
                    phone_type_id = self.pool.get('res.partner.phone.type').search(cr, uid, [('code', '=', 'cel')])
                    phone_ids = phone_obj.search(cr, uid, [('res_partner_id', '=', record.id), ('phone', '=', value), ('type_id', '=' , phone_type_id[0])])
    ...
    def _get_partner(self, cr, uid, ids, context=None):
        result = {}
        for part in self.pool.get('res.partner.phone').browse(cr, uid, ids, context=context):
    ...
    def _get_phone_ids(self, cr, uid, ids, fields, arg, context=None):
        res = {}
        for phone in ids:
            res[phone] = self.pool.get('res.partner.phone').search(cr , uid, [('res_partner_id', '=', phone)])
        return res
