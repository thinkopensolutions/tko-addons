...
class issue_to_task(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        if active_id:
            active_obj = self.pool.get(
                'project.issue').browse(cr, uid, active_id)
    ...
    def convert_to_task(self, cr, uid, ids, context=None):
        active_id = context.get('active_id', False)
        if active_id:
            issue_obj = self.pool.get(
                'project.issue').browse(cr, uid, active_id)
            task_obj = self.pool.get('project.task')
            self_obj = self.browse(cr, uid, ids)[0]
            categs = []
            categ_ids = [categs.append(categ.id)
                         for categ in issue_obj.categ_ids]
            vals = {
                'name': self_obj.name,
            ...
            stage_id = self.pool.get('project.task.type').search(
                cr, uid, ['|', ('name', '=ilike', 'Done'), ('name', '=ilike', u'Concluído')])
            self.pool.get('project.issue').write(
                cr, uid, [active_id], {
                    'task_id': task_id, 'stage_id': stage_id and stage_id[0]}, context=None)

            mod_obj = self.pool.get('ir.model.data')

            res = mod_obj.get_object_reference(
                cr, uid, 'project', 'view_task_form2')
