...
class issue_to_task(osv.osv_memory):
    ...
    def convert_to_task(self, cr, uid, ids, context=None):
        ...
        if active_id:
            ...
            stage_id = self.pool.get('project.task.type').search(cr, uid, ['|', ('name', '=ilike', 'Done'),
                                                                           ('name', '=ilike', u'Concluído')])
            self.pool.get('project.issue').write(cr, uid, [active_id],
                                                 {'task_id': task_id, 'stage_id': stage_id and stage_id[0]},
                                                 context=None)
