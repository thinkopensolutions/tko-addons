...
class crm_claim(osv.osv):
    ...
    _columns = {
        ...
        'survey_ans_id': fields.function(_get_survey_answer, type="many2one", relation='survey.user_input',
                                         string="Survey Answer", fnct_search=_answer_search
                                         ),
        'survey_created': fields.boolean('Survey Created'),
        'answer_state': fields.related('survey_ans_id', 'state', type="selection", selection=AVAILABLE_STATES,
                                       string="Answer State", readonly=True),
    }
    _defaults = {'survey_created': False}

    def onchange_type(self, cr, uid, ids, type, survey, context=None):
        res = {}
        survey_id = False
        survey_ids = []
        if type:
            for survey in self.pool.get('claim.type').browse(cr, uid, type).survey_ids:
                survey_ids.append(survey.id)
            res['value'] = {'survey_id': survey_ids and survey_ids[0]}
        # show all surveys
        if not len(survey_ids):
            survey_ids = self.pool.get('survey.survey').search(cr, uid, [])

        res['domain'] = {'survey_id': [('id', 'in', survey_ids)]}
        return res

    def onchange_survey(self, cr, uid, ids, type, context=None):
        res = {}
        survey_ids = []
        if type:
            for survey in self.pool.get('claim.type').browse(cr, uid, type).survey_ids:
